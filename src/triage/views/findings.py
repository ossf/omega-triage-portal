...
from triage.models.models import Finding, Project, ProjectVersion
...
def api_get_files(request: HttpRequest) -> JsonResponse:
    ...
    source_graph = path_to_graph(package_files + tool_files + intermediate_files, finding.scan.project_version.package_url)
...
def api_get_blob_list(request: HttpRequest) -> JsonResponse:
    ...
    source_graph = path_to_graph(map(lambda s: s.get('relative_path'), source_code), finding.scan.project_version.package_url)
