...
def parse_query_to_Q(query):
    ...
    # Define the grammar
    assigned_to_clause = pp.Group(pp.Keyword("assigned_to").suppress() + pp.Suppress(":") + pp.Word(pp.alphanums).setResultsName("username")).setResultsName("assigned_to")
    severity_clause = pp.Group(pp.Keyword("severity").suppress() + pp.Suppress(":") + pp.delimited_list(pp.one_of(["critical", "important", "moderate", "low"]))).setResultsName("severity")
    updated_dt_clause = pp.Group(pp.Keyword("updated:").suppress() + pp.one_of(["<", ">", "<=", ">=", "=="]).setResultsName("op") + pp.pyparsing_common.iso8601_date("datetime")).setResultsName("updated_dt")
    purl_clause = pp.Group(pp.Keyword("purl").suppress() + pp.Suppress(":") + pp.Word(pp.alphanums + ":@/?=-.").setResultsName("purl")).setResultsName("purl")
